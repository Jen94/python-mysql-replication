...
from tornado_mysql.constants.COMMAND import COM_BINLOG_DUMP, COM_REGISTER_SLAVE
from tornado_mysql.cursors import DictCursor
from tornado_mysql.util import int2byte
...
try:
    from tornado_mysql.constants.COMMAND import COM_BINLOG_DUMP_GTID
except ImportError:
    # Handle old tornado_mysql versions
    # See: https://github.com/tornado_mysql/tornado_mysql/pull/261
...
class BinLogStreamReader(object):
    ...
    def __init__(self, connection_settings, server_id, ctl_connection_settings=None, resume_stream=False,
        ...
        if tornado_mysql_wrapper:
            self.tornado_mysql_wrapper = tornado_mysql_wrapper
        else:
            self.tornado_mysql_wrapper = tornado_mysql.connect
    ...
    def __connect_to_ctl(self):
        ...
        self._ctl_connection = yield self.tornado_mysql_wrapper(**self._ctl_connection_settings)
    ...
    def __checksum_enabled(self):
        ...
        yield cur.execute("SHOW GLOBAL VARIABLES LIKE 'BINLOG_CHECKSUM'")
        result = cur.fetchone()
        cur.close()
    ...
    def __connect_to_stream(self):
        ...
        self._stream_connection = yield self.tornado_mysql_wrapper(**self.__connection_settings)
    ...
    def fetchone(self):
        while True:
            if not self.__connected_stream:
                yield self.__connect_to_stream()

            if not self.__connected_ctl:
                yield self.__connect_to_ctl()

            try:
                if tornado_mysql.__version__ < "0.6":
                    pkt = yield self._stream_connection.read_packet()
                else:
                    pkt = yield self._stream_connection._read_packet()
            except tornado_mysql.OperationalError as error:
            ...
            raise gen.Return(binlog_event.event)
